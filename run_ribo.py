            ...
            all_predictions.append(
                {
                    ...
                    "z": centers_z_test[i],
                    "true_class": y_test[i],
                    "predicted_class": preds.numpy()[i],
                    "probability_class_0": 1 - y_proba[i],
                    "probability_class_1": y_proba[i],
                }
            )
        ...
        results_data.append(
            {
                "tomogram": name,
                "fold": fold_idx + 1,
                "accuracy": all_res_acc[name][fold_idx],
                "auroc": all_res_auroc[name][fold_idx],
                "aupr": all_res_aupr[name][fold_idx],
                "f1": all_res_f1[name][fold_idx],
            }
        )
...
results_df.to_csv(
    "/home/feity/cryoem/temp/ribo_results/classification_results.csv", index=False
)
predictions_df.to_csv(
    "/home/feity/cryoem/temp/ribo_results/particle_predictions.csv", index=False
)
...
for name in all_tomos:
    summary_data.append(
        {
            "tomogram": name,
            "mean_accuracy": np.mean(all_res_acc[name]),
            "std_accuracy": np.std(all_res_acc[name]),
            "mean_auroc": np.nanmean(all_res_auroc[name]),
            "std_auroc": np.nanstd(all_res_auroc[name]),
            "mean_aupr": np.mean(all_res_aupr[name]),
            "std_aupr": np.std(all_res_aupr[name]),
            "mean_f1": np.mean(all_res_f1[name]),
            "std_f1": np.std(all_res_f1[name]),
        }
    )

summary_df = pd.DataFrame(summary_data)
summary_df.to_csv(
    "/home/feity/cryoem/temp/ribo_results/classification_summary.csv", index=False
)

print("\n" + "=" * 60)
print("Results saved:")
print(
    "  - Per-fold results: /home/feity/cryoem/temp/ribo_results/classification_results.csv"
)
print(
    "  - Summary statistics: /home/feity/cryoem/temp/ribo_results/classification_summary.csv"
)
print(
    "  - Particle predictions: /home/feity/cryoem/temp/ribo_results/particle_predictions.csv"
)
print("=" * 60)
